# SQL---Olympics-dataset-for-analysis-and-data-cleaning-
...
Window Functions: Implemented RANK() and PARTITION BY to identify the top-performing athletes for every participating nation.
...
Gold Medal Efficiency: Calculated the "Gold Medal Ratio" to find which countries have the highest percentage of first-place finishes relative to their total medals.

Multi-Sport Mastery: Isolated athletes who successfully competed in more than one distinct sport.

How to Run
Import the athlete_events.csv and noc_regions.csv into a PostgreSQL database.

Execute the queries in SAGAR R SQL final eval project.sql to replicate the analysis.
